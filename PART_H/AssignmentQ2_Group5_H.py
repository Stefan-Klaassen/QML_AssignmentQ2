...
# VEHICLES
electric_vehicle = Vehicle('EV', 300, 120, 1.1, 0.7, 120, 1.25)
print(f"\n{electric_vehicle}")

diesel_vehicle = Vehicle('DV', 300, None, None, None, 100, 2.0)
print(f"\n{diesel_vehicle}")
...
tp = [300, 600, 900, 1200, 1500]                        # break times
cp = 0.5                                                # price per person per time
...
h      = model.addVars(N, V, vtype=GRB.BINARY, name='h')                                    # pause at node i by
...
obj = (
    ...
    quicksum(cc[p] * beta_q[i, p, v] for i in N[1:] for p in P for v in V_EV) +             # Charging costs
    quicksum(cf[v] * z[0, v] for v in V) +                                                  # Fixed outgoing cost
    cp * quicksum(quicksum(x[j,0,v] * (tau_d[j,v] + s * d[j][0] - tau_d[0,v])               # Cost of driver
    for j in N[1:]) for v in V)   
)
